...
export const getGames = async (options?: SpreadsAPIRequest): Promise<Matchup[]> => {
  ...
  return cfbdApi.get<Matchup[]>('games', {
    params: {
    ...
    .then(async (res) => {
      ...
      try {
        ...
        const startDate = add(new Date(weekRange?.start?.startDate), { days: -1 });
        ...
        const spreadsOptions = process.env.REACT_APP_SEASON_KEY === 'offseason' ? {
          ...
          ...options,
          commenceTimeFrom: buildDateFormat((startDate).toISOString()),
        ...
        console.log(resWithUpdatedPropertyNames)
          ...
          .map((item) => {
            ...
            const gameSpread: TheOddsResult | undefined = spreads?.find((spread: TheOddsResult) => {
              ...
              if (!!item.neutralSite && ((strippedHome === strippedAwayTeam && strippedAway === strippedHomeTeam))) {
                ...
                if (spread.home_team === "Boise State Broncos" && spread.away_team === "UNLV Rebels") {

                  if (item.homeTeam === "Boise State" && item.awayTeam === "UNLV") {
                    // force match unlvrebels to unlvrunninrebels
        ...
        console.log('updated matchups with spreads', updated);
