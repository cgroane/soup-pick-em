...
export const getCurrentWeek = async () => {
  try {
    const response = await axiosInstane.get<SeasonDetails>(`/scores/json/CurrentSeasonDetails`)
    return response.data
  } catch (err) {
    console.error(err)
  }
...
export const getGames = async (options?: SpreadsAPIRequest): Promise<Matchup[]> => {
  ...
  const matchupRequestOptions: SpreadsAPIRequest = {
    ...options,
    weekNumber: options?.weekNumber ? options?.weekNumber : season?.ApiWeek.toString(),
    season: options?.season ? options?.season : season?.ApiSeason
  }
  return axiosInstane.get<Matchup[]>(`/stats/json/GamesByWeek/${matchupRequestOptions.season}/${matchupRequestOptions.weekNumber}`, {
