...
export const getRankings = async (year: string, week: string, seasonType: string): Promise<Poll> => {
  return cfbdApi.get<Poll>('/game-data/rankings', { params: { year, week, seasonType } }).then((resp) => resp.data).catch((err) => {
    throw new Error(err)
  })
};

export const getTeams = async (): Promise<GetTeamsResponse> => {
  return cfbdApi.get<GetTeamsResponse>('/game-data/teams', {}).then((response) => {
